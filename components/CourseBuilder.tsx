    ...
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <div className="flex bg-slate-100 rounded-lg p-0.5">
           <button 
             ...
             className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${mode === 'LINK' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-emerald-600'}`}
           >
             Link
           </button>
           <button 
             onClick={() => setMode('UPLOAD')}
             className={`px-3 py-1 text-xs font-bold rounded-md transition-all ${mode === 'UPLOAD' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-emerald-600'}`}
           >
             Upload
           </button>
        </div>
      ...
      {mode === 'LINK' ? (
        <input 
          ...
          className="w-full p-3 border border-slate-200 rounded-xl bg-white text-slate-900 placeholder-slate-400 focus:ring-2 focus:ring-emerald-500 outline-none"
          placeholder={placeholder}
        />
      ) : (
        <div 
          onClick={() => fileInputRef.current?.click()}
          className="w-full p-4 border-2 border-dashed border-slate-300 rounded-xl bg-slate-50 hover:bg-emerald-50 hover:border-emerald-400 transition-all cursor-pointer flex flex-col items-center justify-center text-center"
        >
...
const CourseBuilder: React.FC<CourseBuilderProps> = ({ currentUserHandle, onSubmitCourse }) => {
  ...
  const renderStep1_Info = () => (
    <div className="space-y-6 animate-fade-in">
      <h2 className="text-xl font-bold text-slate-800 border-b border-slate-100 pb-4">Step 1: Course Information</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-1">Course Track (Category)</label>
            <select 
              value={course.track} 
              onChange={e => updateCourseInfo('track', e.target.value)}
              className="w-full p-3 border border-slate-200 rounded-xl bg-white text-slate-900 outline-none"
            >
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea 
              ...
              className="w-full p-3 border border-slate-200 rounded-xl bg-white text-slate-900 placeholder-slate-400 focus:ring-2 focus:ring-emerald-500 outline-none h-32"
  ...
  const renderStep2_Curriculum = () => (
    <div className="space-y-6 animate-fade-in h-full flex flex-col">
      <div className="flex justify-between items-center border-b border-slate-100 pb-4">
        <h2 className="text-xl font-bold text-slate-800">Step 2: Curriculum Builder</h2>
        <button onClick={addModule} className="text-sm bg-emerald-50 text-emerald-700 px-4 py-2 rounded-lg font-bold hover:bg-emerald-100 transition-colors">
          + Add Module
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-6 pr-2">
        ...
        {course.modules.map((module, mIdx) => (
          <div key={module.id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
             {/* Module Header */}
             <div className="bg-slate-50 p-4 flex flex-col sm:flex-row sm:items-center gap-4 border-b border-slate-100">
                <span className="font-bold text-slate-500 whitespace-nowrap">Module {mIdx + 1}:</span>
                <input 
                  type="text" 
                  value={module.title}
                  onChange={(e) => updateModule(module.id, 'title', e.target.value)}
                  className="flex-1 bg-white border border-slate-200 rounded-lg px-3 py-2 focus:border-emerald-500 outline-none text-slate-900 font-medium placeholder-slate-400"
                  placeholder="Enter Module Title"
                />
                <button onClick={() => deleteModule(module.id)} className="text-slate-400 hover:text-red-500"><TrashIcon /></button>
                <button onClick={() => addChapter(module.id)} className="text-xs bg-white border border-slate-200 px-3 py-2 rounded-lg text-slate-600 hover:text-emerald-600 hover:border-emerald-200 font-bold shadow-sm">
                  + Add Chapter
                </button>
             </div>
             ...
             <div className="p-2 space-y-2">
                {module.chapters.length === 0 && <p className="text-xs text-slate-400 text-center py-2">No chapters yet.</p>}
                {module.chapters.map((chapter, cIdx) => (
                  <div key={chapter.id} className="flex items-center justify-between p-3 hover:bg-slate-50 rounded-lg group border border-transparent hover:border-slate-100 transition-all">
                     <div className="flex items-center gap-3">
                        <div className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-500">{cIdx + 1}</div>
                     ...
                     <div className="flex items-center gap-2 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                        <button 
                            ...
                            className="text-xs text-emerald-600 font-bold hover:text-emerald-800 bg-emerald-50 px-3 py-1.5 rounded-lg"
                        >
                            Edit Content
                        </button>
                        <button 
                            onClick={(e) => deleteChapter(e, module.id, chapter.id)}
                            className="text-xs text-red-500 font-bold hover:text-red-700 bg-red-50 px-2 py-1.5 rounded-lg"
  ...
  const renderStep3_Settings = () => (
    <div className="space-y-6 animate-fade-in">
      <h2 className="text-xl font-bold text-slate-800 border-b border-slate-100 pb-4">Step 3: Enhancements & Settings</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
         <div className="space-y-4">
             <h3 className="font-bold text-emerald-900">Visibility & Access</h3>
             <label className="flex items-center justify-between p-4 border border-slate-200 rounded-xl bg-white">
                <span className="text-sm font-medium text-slate-700">Team Only (Private)</span>
                <input 
                  type="checkbox" 
                  checked={course.settings.teamOnly} 
                  onChange={e => updateSettings('teamOnly', e.target.checked)} 
                  className="w-5 h-5 text-emerald-600 rounded border-slate-300 bg-white accent-emerald-600" 
                />
             </label>
         </div>

         <div className="space-y-4">
             <h3 className="font-bold text-emerald-900">Gamification</h3>
             <label className="flex items-center justify-between p-4 border border-slate-200 rounded-xl bg-white">
                <span className="text-sm font-medium text-slate-700">Enable Certificates</span>
                <input 
                  type="checkbox" 
             ...
             <div className="flex items-center justify-between p-4 border border-slate-200 rounded-xl bg-white">
                <span className="text-sm font-medium text-slate-700">Points Reward</span>
                <input 
                  type="number" 
                  value={course.settings.pointsReward} 
                  onChange={e => updateSettings('pointsReward', parseInt(e.target.value))} 
                  className="w-20 p-2 text-center border border-slate-200 rounded-lg text-slate-900 bg-slate-50 focus:ring-2 focus:ring-emerald-500 outline-none"
                />
             </div>
         </div>
      </div>
    ...
    return (
      <div className="fixed inset-0 z-50 flex justify-end">
         ...
         <div className="relative w-full max-w-2xl bg-white h-full shadow-2xl flex flex-col animate-slide-in-right">
            <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-emerald-900 text-white">
               <div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50">
                <div>
                    ...
                    <input 
                        ...
                        className="w-full p-3 border border-slate-200 rounded-xl bg-white text-slate-900 focus:border-emerald-500 outline-none"
                    />
                </div>
                ...
                <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">Chapter Summary (Preview)</label>
                    <textarea 
                        value={activeChapter.summary || ''} 
                        onChange={e => updateActiveChapter('summary', e.target.value)}
                        className="w-full p-3 border border-slate-200 rounded-xl bg-white text-slate-900 h-20 resize-none focus:border-emerald-500 outline-none"
                    />
                </div>

                <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">Main Content (Markdown)</label>
                    <textarea 
                        value={activeChapter.content} 
                        onChange={e => updateActiveChapter('content', e.target.value)}
                        className="w-full p-3 border border-slate-200 rounded-xl bg-white text-slate-900 h-64 font-mono text-sm focus:border-emerald-500 outline-none"
  ...
  return (
    <div className="max-w-5xl mx-auto h-auto md:h-[calc(100vh-6rem)] flex flex-col">
      ...
      <div className="flex items-center justify-between mb-6 bg-white p-4 rounded-xl shadow-sm border border-slate-100">
         <div className="flex items-center gap-3">
             <button onClick={() => navigate('/dashboard')} className="text-slate-500 hover:text-emerald-700 transition-colors p-1 rounded-lg hover:bg-emerald-50">
                 <ArrowLeftIcon />
             </button>
             <div>
                 <h1 className="font-bold text-xl text-emerald-950 font-heading">Course Builder</h1>
      ...
      <div className="flex flex-col md:flex-row h-full gap-6 overflow-hidden">
        ...
        {/* Sidebar Steps */}
        <div className="w-full md:w-64 bg-white rounded-2xl shadow-sm border border-slate-100 p-4 flex md:flex-col gap-2 overflow-x-auto md:overflow-visible shrink-0 h-fit md:h-full">
           {[
             ...
             <button 
               ...
               className={`flex items-center gap-3 p-3 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${
                 step === s.id ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'
               }`}
             >
               <span>{s.icon}</span>
               <span>{s.label}</span>
        ...
        {/* Main Content Area */}
        <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 overflow-y-auto relative min-h-[500px]">
            ...
            {step === 4 && (
              <div className="space-y-6 animate-fade-in text-center py-10">
                  ...
                  <div className="bg-slate-50 p-6 rounded-xl text-left max-w-md mx-auto border border-slate-200 mt-6">
                      <h4 className="font-bold text-slate-700 mb-2">Summary</h4>
                      <p className="text-sm text-slate-600"><span className="font-bold">Title:</span> {course.title}</p>
                      <p className="text-sm text-slate-600"><span className="font-bold">Track:</span> {course.track}</p>
                      <p className="text-sm text-slate-600"><span className="font-bold">Level:</span> {course.level}</p>
                  </div>
      ...
      <div className="flex justify-between mt-6 md:hidden pb-6">
          ...
          <button 
            ...
            className="text-emerald-600 font-bold disabled:opacity-30"
          >
            Next &rarr;
          </button>
      </div>
