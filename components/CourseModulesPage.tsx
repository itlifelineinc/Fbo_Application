...
import { ArrowLeft, PlayCircle, Layers, CheckCircle, ChevronLeft } from 'lucide-react';
...
const CourseModulesPage: React.FC<CourseModulesPageProps> = ({ courses, completedModules }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-950 animate-fade-in">
      ...
      <div className="md:hidden sticky top-0 z-40 bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 px-4 py-3 flex items-center gap-3 shrink-0">
        <button 
            onClick={() => navigate(-1)} 
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 space-y-8 pb-24 md:pb-8">
        ...
        <div className="hidden md:flex flex-col md:flex-row gap-6 items-start md:items-center">
            <button 
                onClick={() => navigate(-1)}
