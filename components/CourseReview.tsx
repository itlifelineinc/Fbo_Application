...
const CourseReview: React.FC<CourseReviewProps> = ({ courses, onReviewCourse }) => {
  ...
  if (!courses) {
    return <div className="p-10 text-center text-slate-500">Loading course data...</div>;
  }
    ...
    <div className="space-y-8 animate-fade-in pb-10">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="h-32 bg-slate-100 relative">
                ...
                <div className="absolute -bottom-10 left-8 w-24 h-24 bg-white rounded-xl p-1 shadow-md border border-slate-200">
                    <img src={course.thumbnailUrl} className="w-full h-full object-cover rounded-lg bg-slate-100" alt="Thumbnail" />
                </div>
            </div>
            <div className="pt-12 px-8 pb-8">
                <div className="flex justify-between items-start">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 font-heading">{course.title}</h1>
                        <p className="text-slate-500 text-lg mt-1">{course.subtitle}</p>
                    </div>
                    <div className="text-right">
                        <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">{course.status}</span>
                        <p className="text-xs text-slate-400 mt-2">Author: <span className="font-mono text-slate-600">{course.authorHandle}</span></p>
                    </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
                ...
                <Section title="Learning Outcomes">
                    ...
                    ) : <p className="text-slate-400 italic">No outcomes specified.</p>}
                </Section>
            </div>
            ...
            <div className="space-y-8">
                 <Section title="Target Audience">
                    <div className="flex flex-wrap gap-2">
                        {course.targetAudience.length > 0 ? course.targetAudience.map((aud, i) => (
                            <span key={i} className="bg-slate-100 text-slate-600 px-3 py-1 rounded-full text-xs font-medium">{aud}</span>
                 ...
                 <Section title="Configuration">
                    <div className="space-y-3">
                        ...
                        <div className="flex justify-between pt-2 border-t border-slate-100">
                            ...
                            <span className="font-bold text-emerald-600">{course.settings.pointsReward} XP</span>
                        </div>
                         <div className="flex justify-between">
                            <span className="text-sm text-slate-500">Price</span>
                            <span className="font-bold text-slate-800">{course.settings.price ? `$${course.settings.price}` : 'Free'}</span>
                        </div>
                    </div>
                 </Section>
                 ...
                 {course.trailerVideoUrl && (
                     <Section title="Trailer Video">
                        <a href={course.trailerVideoUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-blue-600 hover:underline text-sm">
  ...
  const renderChapterDetail = (chapter: Chapter) => (
      <div className="max-w-3xl mx-auto bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden animate-fade-in">
            ...
            <div className="p-8 space-y-8">
                {/* Summary */}
                {chapter.summary && (
                    <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-r-lg text-blue-900 italic text-sm">
                        "{chapter.summary}"
                    </div>
                )}
                ...
                {chapter.actionSteps && chapter.actionSteps.length > 0 && (
                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
                            <h3 className="font-bold text-slate-800 mb-3 text-xs uppercase tracking-wide flex items-center gap-2">
                ...
                {/* Main Content */}
                <div>
                    <h3 className="font-bold text-slate-800 mb-3 border-b border-slate-100 pb-2">Lesson Content</h3>
                ...
                {chapter.quizQuestions && chapter.quizQuestions.length > 0 && (
                    <div className="mt-8 pt-8 border-t border-slate-200">
                        <h3 className="font-bold text-slate-800 mb-4">Quiz Questions ({chapter.quizQuestions.length})</h3>
                        <div className="space-y-4">
                            {chapter.quizQuestions.map((q, i) => (
                                <div key={i} className="bg-slate-50 p-4 rounded-lg border border-slate-200">
                                    <p className="font-medium text-sm text-slate-800 mb-2"><span className="text-slate-400 mr-2">Q{i+1}.</span> {q.question}</p>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        {q.options.map((opt, optIdx) => (
                                            <div key={optIdx} className={`text-xs px-3 py-2 rounded border ${optIdx === q.correctAnswer ? 'bg-green-50 border-green-200 text-green-700 font-bold' : 'bg-white border-slate-200 text-slate-500'}`}>
    ...
    <div className="h-[calc(100vh-4rem)] flex flex-col bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-3 flex justify-between items-center shadow-sm shrink-0 z-20">
         <div className="flex items-center gap-4">
             ...
             <div>
                 <h1 className="font-bold text-lg text-slate-800 font-heading">Course Review Portal</h1>
                 <p className="text-xs text-slate-500">Submited by {course.authorHandle}</p>
             </div>
         </div>
         <div className="flex gap-3">
             <button 
                onClick={handleReject}
                className="px-4 py-2 rounded-lg text-sm font-bold text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 transition-colors"
             >
                Reject Course
             </button>
             <button 
         ...
         <div className="w-80 bg-white border-r border-slate-200 flex flex-col overflow-y-auto shrink-0">
            <div className="p-4 border-b border-slate-100">
                <button
                    ...
                    className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all flex items-center gap-3 ${activeView === 'OVERVIEW' ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200' : 'text-slate-600 hover:bg-slate-50'}`}
                >
            ...
            <div className="p-4">
                <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-3 ml-2">Curriculum Content</h4>
                <div className="space-y-6">
                    {course.modules.map((module, idx) => (
                        <div key={module.id}>
                            <div className="flex items-center gap-2 mb-2 px-2 text-slate-800 font-bold text-xs uppercase tracking-wide">
                                <span className="bg-slate-100 text-slate-500 w-5 h-5 flex items-center justify-center rounded-full">{idx + 1}</span>
                                {module.title}
                            </div>
                            <div className="space-y-1 pl-2 border-l-2 border-slate-100 ml-2.5">
...
const ConfigRow: React.FC<{label: string, value: boolean}> = ({ label, value }) => (
    <div className="flex justify-between items-center text-sm">
        ...
        <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${value ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-400'}`}>
            {value ? 'On' : 'Off'}
        </span>
    </div>
);
