  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <label className="block text-sm font-bold text-slate-700">Single Products</label>
        <button onClick={addProduct} className="text-xs flex items-center gap-1 text-emerald-600 font-bold hover:bg-emerald-50 px-2 py-1 rounded">
      ...
      <div className="space-y-3">
        {data.products.map((product) => (
          <div key={product.id} className="border border-slate-200 rounded-xl bg-white overflow-hidden shadow-sm transition-all">
            {/* Header */}
            <div 
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">{data.currency} {product.price}</p>
                </div>
              </div>
              ...
              <div className="p-4 border-t border-slate-100 bg-slate-50/50 space-y-4 animate-fade-in">
                ...
                <div className="flex gap-4">
                  <div 
                    ...
                    className="w-24 h-24 bg-slate-100 border-2 border-dashed border-slate-300 rounded-xl flex flex-col items-center justify-center text-slate-400 cursor-pointer hover:border-emerald-400 hover:text-emerald-500 transition-colors shrink-0"
                  >
                    {product.image ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 mb-1">Price ({data.currency})</label>
                    <input 
                      type="number" 
                      value={product.price}
                ...
                {/* Description */}
                <div>
                  <label className="block text-xs font-bold text-slate-500 mb-1">Full Description</label>
                  <textarea 
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <label className="text-xs font-bold text-slate-500">Key Benefits</label>
                    <button 
