  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <span className="text-xs text-slate-400">Visual Editor</span>
      </div>
      <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm focus-within:ring-2 focus-within:ring-emerald-500 transition-all flex flex-col h-96">
        {/* Toolbar */}
        <div className="bg-slate-50 border-b border-slate-200 px-2 py-2 flex flex-wrap gap-1 items-center shrink-0">
...
const ToolbarButton: React.FC<{ onClick: (e: React.MouseEvent) => void; icon: React.ReactNode; label: string }> = ({ onClick, icon, label }) => (
    <button 
        ...
        type="button"
        className="p-1.5 text-slate-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-lg transition-colors flex items-center gap-1.5"
        title={label}
    >
        {icon}
