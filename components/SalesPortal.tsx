...
const SalesPortal: React.FC<SalesPortalProps> = ({ currentUser, onSubmitSale }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert(`Sale submitted! You earned ${currentCC} CC.`);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      <header className="md:flex md:items-center md:justify-between">
        <div>
            <h1 className="text-3xl font-bold text-emerald-950 font-heading">Sales & Case Credits</h1>
            <p className="text-emerald-700 mt-1">Report your sales to rank up in the business.</p>
        </div>
        <div className="mt-4 md:mt-0 bg-emerald-100 text-emerald-800 px-6 py-3 rounded-xl font-bold flex flex-col items-center border border-emerald-200">
            <span className="text-xs uppercase tracking-wider text-emerald-600">Total Accumulated</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8">
            <h2 className="text-xl font-bold text-slate-800 mb-6 font-heading">Log New Transaction</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div 
                    ...
                    className={`border-2 border-dashed rounded-xl p-6 text-center cursor-pointer transition-all ${
                        receiptImage ? 'border-emerald-500 bg-emerald-50' : 'border-slate-300 hover:border-emerald-400 hover:bg-slate-50'
                    }`}
                >
                    {isScanning ? (
                        <div className="py-4 flex flex-col items-center text-emerald-600">
                        ...
                        <div className="relative">
                            ...
                            <p className="text-xs text-emerald-600 mt-2 font-medium">Click to change image</p>
                        </div>
                    ) : (
                        <div className="py-4 text-slate-500">
                            <CameraIcon />
                            <p className="text-sm font-medium mt-2">Upload Receipt / Proof of Payment</p>
                            <p className="text-xs text-slate-400 mt-1">We'll scan the amount automatically</p>
                        </div>
                    )}
                    <input 
                        type="file" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-slate-700 mb-2">Sale Type</label>
                        <select 
                            value={saleType}
                ...
                <div className="bg-slate-50 p-4 rounded-xl flex justify-between items-center border border-slate-200">
                    <div>
                        <p className="text-xs text-slate-500 uppercase font-bold">Calculated Value</p>
                        <p className="text-sm text-slate-600 mt-1">
                            ${amount || '0'} / ${saleType === 'RETAIL' ? RETAIL_DIVISOR : WHOLESALE_DIVISOR}
                        </p>
                    </div>
                    <div className="text-right">
                        <p className="text-3xl font-bold text-emerald-600 font-heading">{currentCC}</p>
                        <p className="text-xs text-emerald-800 font-bold">CC Earned</p>
                    </div>
                </div>
                    ...
                    {currentUser.salesHistory && currentUser.salesHistory.length > 0 ? (
                        currentUser.salesHistory.map((sale) => (
                            <div key={sale.id} className="p-4 flex justify-between items-center hover:bg-slate-50">
                                <div>
                                    <p className="font-bold text-slate-800 text-sm">{sale.type} Sale</p>
                                    <p className="text-xs text-slate-500 font-mono">{sale.transactionId}</p>
                                    <p className="text-xs text-slate-400">{sale.date}</p>
                                </div>
                                <div className="text-right">
                                    <span className="block font-bold text-emerald-600">+{sale.ccEarned} CC</span>
